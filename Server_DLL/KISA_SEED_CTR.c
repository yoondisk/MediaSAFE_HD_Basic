...
int SEED_CTR_Process( OUT KISA_SEED_INFO *pInfo, IN DWORD *in, IN int inLen, OUT DWORD *out, OUT int *outLen )
{
	...
	{
		KISA_SEED_Block_forCTR( (unsigned int *)pdwCounter,(unsigned int *) out, &pInfo->seed_key );
...
int SEED_CTR_Encrypt( IN BYTE *pbszUserKey, IN BYTE *pbszCTR, IN BYTE *pbszInputText, IN int nInputTextLen, OUT BYTE *pbszOutputText )
{
	...
	data =(unsigned int *) chartoint32_for_SEED_CTR(newpbszInputText, nInputTextLen );

	SEED_CTR_Process( &info, ( DWORD *)data, nInputTextLen,(DWORD *) outbuf, &nOutLeng );
	SEED_CTR_Close( &info, ( DWORD *)( outbuf+(nOutLeng)/4), &nEncRmainLeng);
	cdata = int32tochar_for_SEED_CTR((DWORD *)outbuf, nOutLeng - nEncRmainLeng);
	memcpy(pbszOutputText, cdata, nOutLeng - nEncRmainLeng);

	free(data);
	free(cdata);
	free(outbuf);
	free(newpbszInputText);
