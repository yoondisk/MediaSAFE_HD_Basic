...
int web_run()//메인함수 
{
	...
	if( listen(listenfd,1000) <0){//클라이언트 연결 기다리기 
	...
	for(hit=1;;hit++){//while문과 동일 
		...
		if((pid = fork()) < 0) {//안열리면 
			...
			if(pid == 0) {////////////////////fork시작 
				...
				//log(buffer,file_name,strlen(buffer)); //로그작성 
				...
				if(file_fd==-1){//파일이 아니라면? 
					if(strstr(file_name,"key.htm")){ //받은 파일 이름에 &end 있다면? 
						sprintf(buffer,"HTTP/1.1 200 OK\r\nContent-Type: text/html\r\n\r\n1234567890123456");//200으로 헤더를 설정 
						log(buff,file_name,s.st_size); //로그작성 
						write(socketfd,buffer,strlen(buffer));//socekfd에 버퍼를 써준다. 
						exit(1);//나간다. 
					}
					
					if(strstr(file_name,"&end")){ //받은 파일 이름에 &end 있다면? 
						// http://127.0.0.1:8080/&end <-- 서버종료.
